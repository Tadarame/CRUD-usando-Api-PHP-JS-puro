...
let usersCache = [];
//guarda a ultima lista buscada

export function findUserById(id){
    return usersCache.find((user) => user.id === id);
    //aqui ele procura no cache de user se o id é igual ao id do usuario procurado
        ...
        userDiv.innerHTML = `
            <div class="card user-card h-100" data-id = "${user.id}">
                ...
                <div class = "card-footer d-flex gap-2">
                    ...
                    <button class = "btn btn-sm btn-outline-dark flex-fill" data-action = "delete">Delete</button>
                </div>
            </div>
        `;
