...
} //cria uma função de repsota que recebe

function handleGet(string $dataFile) :void
{
    try {
        echo json_enconde(getAllUsers($datafile));
    } catch (\Throwable $e){
...
function handlePatch(string $dataFile) :void
{
    try {
        ...
        http_response_code(500);
        echo json_encode(['error' => 'internal server']);
    }
}
function handleDelete(string $dataFile) :void
{
    try {
        $id = isset($_Get['id']) ? (int) $_GET['id'] : null;
        respond(removerUser($dataFile, $id));
    } catch (\throwable $e) {
        http_response_code(500);
        echo json_encode(['error' => 'internal server']);
    }
...
function handleMethodNotAllowed() :void
{
    ...
    echo json_encode(['error' => 'Method not Alloowed']);
}